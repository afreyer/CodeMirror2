...
// Modded merge plugin for NCLab.
// Supports the 'ignoreWhitespaces' option (off by default)
...
(function(mod) {
  ...
  function registerUpdate(dv) {
    ...
    function update(mode) {
      ...
      if (dv.showDifferences) {
        updateMarks(dv.edit, dv.diff, dv.diffIW, edit, DIFF_INSERT, dv.classes);
        updateMarks(dv.orig, dv.diff, dv.diffIW, orig, DIFF_DELETE, dv.classes);
      }
      drawConnectors(dv);
    }
    function set(slow) {
  ...
  function syncScroll(dv, type) {
    ...
    var around = chunkBoundariesAround(dv.diff, dv.diffIW, mid, type == DIFF_INSERT, dv.mv.options.ignoreWhitespaces);
  ...
  function updateMarks(editor, diff, diffIW, state, type, classes) {
    var vp = editor.getViewport();
    editor.operation(function() {
      if (state.from == state.to || vp.from - state.to > 20 || state.from - vp.to > 20) {
        clearMarks(editor, state.marked, classes);
        markChanges(editor, diff, diffIW, type, state.marked, vp.from, vp.to, classes);
        state.from = vp.from; state.to = vp.to;
      } else {
        if (vp.from < state.from) {
          markChanges(editor, diff, diffIW, type, state.marked, vp.from, state.from, classes);
          state.from = vp.from;
        }
        if (vp.to > state.to) {
          markChanges(editor, diff, diffIW, type, state.marked, state.to, vp.to, classes);
          state.to = vp.to;
        }
      }
    });
  ...
  function markChanges(editor, diff, diffIW, type, marks, from, to, classes) {
    var pos = Pos(0, 0);
    var top = Pos(from, 0), bot = editor.clipPos(Pos(to - 1));
    var cls = type == DIFF_DELETE ? classes.del : classes.insert;
    var ignoreWhitespaces = editor.options.ignoreWhitespaces;
    ...
    // skip marking chunks, only decorate text with underlines for now
    var chunkStart = 0;
    ...
    // only mark chunks now, using a specific diff version
    var chunkdiff = ignoreWhitespaces ? diffIW : diff;
    
    var pos = Pos(0, 0);
    var chunkStart = 0;
    for (var i = 0; i < chunkdiff.length; ++i) {
      var part = chunkdiff[i], tp = part[0], str = part[1];
      if (tp == DIFF_EQUAL) {
        var cleanFrom = pos.line + (startOfLineClean(chunkdiff, i) ? 0 : 1);
        moveOver(pos, str);
        var cleanTo = pos.line + (endOfLineClean(chunkdiff, i) ? 1 : 0);
        if (cleanTo > cleanFrom) {
           if (i) markChunk(chunkStart, cleanFrom);
          chunkStart = cleanTo;
        }
...
//            marks.push(editor.markText(a, b, {className: cls}));
  ...
  function drawConnectors(dv) {
    ...
    iterateChunks(dv.diff, dv.diffIW, function(topOrig, botOrig, topEdit, botEdit) {
    ...
    }, dv.mv.options.ignoreWhitespaces);
  }
  ...
  function getDiff(a, b, ignoreWhitespaces) {
  ...
  function iterateChunks(diff, diffIW, f, ignoreWhitespaces) {
    var startEdit = 0, startOrig = 0;
    var edit = Pos(0, 0), orig = Pos(0, 0);
    
    diff = ignoreWhitespaces ? diffIW : diff;
  ...
  function chunkBoundariesAround(diff, diffIW, n, nInEdit, ignoreWhitespaces) {
    var beforeE, afterE, beforeO, afterO;
    iterateChunks(diff, diffIW, function(fromOrig, toOrig, fromEdit, toEdit) {
